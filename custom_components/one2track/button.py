...
from .const import CMD_FIND_DEVICE, CMD_REFRESH_LOCATION, CMD_REMOTE_SHUTDOWN, LOGGER
...
class One2TrackButton(One2TrackEntity, ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        if desc.cmd_code == CMD_REMOTE_SHUTDOWN:
            LOGGER.warning(
                "Remotely shutting down device %s — cannot be re-enabled remotely!",
                self._uuid,
            )
